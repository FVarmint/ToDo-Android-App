...
public class MainFragment extends AppDefaultFragment {
    ...
    public void onViewCreated(View view, @Nullable Bundle savedInstanceState) {
        ...
        // Recover the theme, and setTheme accordingly
        ...
        /*
            Implement and instantiate custom scroll listener.
         */
        customRecyclerScrollViewListener = new CustomRecyclerScrollViewListener() {
            @Override
            public void show() {
                // Show the action button when the recyclerview stops scrolling
                // Buggy, the button doesn't pop back when the recyclerview reaches the bottom.
                mAddToDoItemFAB.animate().translationY(0).setInterpolator(new DecelerateInterpolator(2)).start();
            }

            @Override
            public void hide() {
                // Hide the action button when the recyclerview is being scrolled.
        ...
        // Use this scroll listener as the main scroll listener for the recyclerview
    ...
    public void onResume() {
        ...
        /*
        We need to do this, as this activity's onCreate won't be called when coming back from
        SettingsActivity, thus our changes to dark/light mode won't take place, as the
        setContentView() is not called again. So, inside our SettingsFragment, whenever the
        checkbox's value is changed, in our shared preferences, we mark our recreate_activity key
        as true.

        Note: the recreate_key's value is changed to false before calling recreate(), or we would
        have ended up in an infinite loop, as onResume() will be called on recreation, which will
        again call recreate() and so on.... and get an ANR
         */
        if (getActivity().getSharedPreferences(THEME_PREFERENCES, MODE_PRIVATE)
                .getBoolean(RECREATE_ACTIVITY, false)) {
            SharedPreferences.Editor editor = getActivity()
                    .getSharedPreferences(THEME_PREFERENCES, MODE_PRIVATE).edit();
